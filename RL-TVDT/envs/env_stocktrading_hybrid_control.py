...
# from TVDT.transformer import Transformer_base as PredictionModel
from TVDT.transformer_dff_128 import Transformer_base as PredictionModel_1
from envs.models.transformer import Transformer_base as PredictionModel_2
...
class StockTradingEnv(gym.Env):
    ...
    def __init__(
        ...
        self.long_prediction_model = self.load_model_1(long_prediction_model_path).to(self.device)
    ...
    def load_model(self, path, enc_in=10, dec_in=10, c_out=1):
        model = PredictionModel_2(enc_in=enc_in, dec_in=dec_in, c_out=c_out)
        ...
        if path is not None:
            ...
            new_state_dict = OrderedDict()
            for k, v in state_dict.items():
                new_key = k.replace("module.", "")  # 处理 DataParallel 的 key
                new_state_dict[new_key] = v

            try:
                model.load_state_dict(new_state_dict, strict=False)
                print("Successfully loaded prediction model:", path)
            except RuntimeError as e:
                print(f"Error loading state_dict: {e}")
                print("Check if model architecture matches the state_dict.")
        return model

    def load_model_1(self, path, enc_in=10, dec_in=10, c_out=1):
        model = PredictionModel_1(enc_in=enc_in, dec_in=dec_in, c_out=c_out)
