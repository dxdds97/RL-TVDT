...
class BaseCallback(ABC):
    ...
    def __init__(self, verbose: int = 0):
        ...
        # An alias for self.TVDT-enc1.get_env(), the environment used for training
    ...
    def init_callback(self, model: "base_class.BaseAlgorithm") -> None:
        ...
        RLmodels and the training environment for convenience.
        """
        self.model = model
    ...
    def on_step(self) -> bool:
        """
        This method will be called by themodels after each call to ``env.step()``.
...
class CheckpointCallback(BaseCallback):
    """
    Callback for saving amodels every ``save_freq`` calls
    to ``env.step()``.
    ...
    :param save_freq:
    :param save_path: Path to the folder where themodels will be saved.
    :param name_prefix: Common prefix to the savedmodels
    :param verbose:
    """
    ...
    def _on_step(self) -> bool:
        if self.n_calls % self.save_freq == 0:
            ...
            if self.verbose > 1:
                print(f"Saving models checkpoint to {path}")
        return True
...
class EvalCallback(EventCallback):
    ...
    :param callback_on_new_best: Callback to trigger
        when there is a new bestmodels according to the ``mean_reward``
    :param n_eval_episodes: The number of episodes to test the agent
    :param eval_freq: Evaluate the agent every ``eval_freq`` call of the callback.
    :param log_path: Path to a folder where the evaluations (``evaluations.npz``)
        will be saved. It will be updated at each evaluation.
    :param best_model_save_path: Path to a folder where the bestmodels
    ...
    def _on_step(self) -> bool:
        ...
        if self.eval_freq > 0 and self.n_calls % self.eval_freq == 0:
            ...
            self.model.save(os.path.join(self.best_model_save_path, "model"+str(self.model_index*1000)))   # TVDT-enc1
            self.model_index +=1
...
class StopTrainingOnRewardThreshold(BaseCallback):
    ...
    has been reached (i.e. when themodels is good enough).
...
class StopTrainingOnMaxEpisodes(BaseCallback):
    ...
    def _on_step(self) -> bool:
        ...
        # Somemodels use keyword 'done' (e.g.,: SAC, TD3, DQN, DDPG)
        # While somemodels use keyword 'dones' (e.g.,: A2C, PPO)
        ...
        if self.verbose > 0 and not continue_training:
            ...
            print(
                ...
                f"{self.locals.get('tb_log_name')}models reached max_episodes={self.max_episodes}, "
