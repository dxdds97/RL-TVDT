...
used to serialize data (class parameters) of TVDT classes
"""
import base64
import functools
import io
...
def json_to_data(json_string: str, custom_objects: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    :param custom_objects: Dictionary of objects to replace
        ...
        ``keras.TVDT.load_model``. Useful when you have an object in
...
def save_to_zip_file(
    ...
    """
    Savemodels data to a zip archive.

    :param save_path: Where to store themodels.
...
def load_from_zip_file(
    ...
    """
    Loadmodels data from a .zip archive

    :param load_path: Where to load themodels from
    :param load_data: Whether we should load and return data
        (class parameters). Mainly used by 'load_parameters' to only loadmodels parameters (weights)
    :param custom_objects: Dictionary of objects to replace
        upon loading. If a variable is present in this dictionary as a
        key, it will not be deserialized and the corresponding item
        will be used instead. Similar to custom_objects in
    ...
    :param print_system_info: Whether to print or not the system info
        about the savedmodels.
    :return: Class parameters,TVDT state_dicts (aka "params", dict of state_dict)
        ...
        with zipfile.ZipFile(load_path) as archive:
            ...
            if print_system_info:
                if "system_info.txt" in namelist:
                    ...
                    warnings.warn(
                        "Themodels was saved with SB3 <= 1.2.0 and thus cannot print system information.",
                        UserWarning,
                    )
