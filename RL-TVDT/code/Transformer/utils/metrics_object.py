...
import utils
from utils.metrics import mirr_top1, mirr_top_k, rank_ic, mae, mse, RSE, covariance, correlation, mape, mspe
...
class RSEMetric(MetricMeter):
    def __init__(self, postfix=''):
        if postfix != '':
            postfix = '/' + postfix
        super(RSEMetric, self).__init__(f"rse{postfix}", RSE)



class Covariance(MetricMeter):
    def __init__(self, postfix=''):
        if postfix != '':
            postfix = '/' + postfix
        super(Covariance, self).__init__(f"covariance{postfix}",covariance)

    def __str__(self):
        return f"{self.mean():.6f}"

class Correlation(MetricMeter):
    def __init__(self, postfix=''):
        if postfix != '':
            postfix = '/' + postfix
        super(Correlation, self).__init__(f"correlation{postfix}",correlation)

    def __str__(self):
        return f"{self.mean():.6f}"

class MSPE(MetricMeter):
    def __init__(self, postfix=''):
        if postfix != '':
            postfix = '/' + postfix
        super(MSPE, self).__init__(f"mspe{postfix}", mspe)

    def __str__(self):
        return f"{self.mean():.6f}"


class MAPE(MetricMeter):
    def __init__(self, postfix=''):
        if postfix != '':
            postfix = '/' + postfix
        super(MAPE, self).__init__(f"mape{postfix}", mape)
