...
class FeatureEngineer:
    ...
    def add_technical_indicator(self, data):
        ...
        window = 14
        # # calculate VWAP
        df['vwap'] = (df['Typical Price'] * df['volume']).rolling(window=window).sum() / df['volume'].rolling(
            window=window).sum()

        # VWAP std
        df['vwap_std'] = df['vwap'].rolling(window=window).std()
        df['vwap_std'] = df['vwma'].rolling(window=window).std()
        ...
        df['vwap_std'].fillna(method='bfill', inplace=True)  #
