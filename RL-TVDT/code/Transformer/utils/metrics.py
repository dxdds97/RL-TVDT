...
def RSE(pred: torch.Tensor, true: torch.Tensor) -> float:
    result = torch.sqrt(torch.sum((true - pred) ** 2)) / torch.sqrt(torch.sum((true - true.mean()) ** 2))
    return result.detach().cpu().item()  # 确保返回 Python float
...
def mape(pred, true):
    epsilon = 1e-8  # 防止除以零
    result = torch.mean(torch.abs((pred - true) / (true + epsilon)))
    return result.detach().cpu().item()

def mspe(pred, true):
    epsilon = 1e-8  # 防止除以零
    result = torch.mean(torch.square((pred - true) / (true + epsilon)))
    return result.detach().cpu().item()
