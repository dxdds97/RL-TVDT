...
class Exp_pred(Exp_Basic):
    ...
    def _build_model(self):
        ...
        if self.args.model=='Transformer':
            model = model_dict[self.args.model](  # 实例Transformer
                # self.args
                self.args.enc_in,
                self.args.dec_in, 
                self.args.c_out,
    ...
    def train(self, setting):
        ...
        train_steps = len(train_loader)        # 批次数量235
        model_optim = self._select_optimizer()  # 选择优化器Adam
        criterion =  self._select_criterion()  # 目标函数MSE
        ...
        for epoch in range(self.args.train_epochs):
            ...
            for i, (batch_x1, batch_x2, batch_y) in enumerate(train_loader):    # 特征x1,x2,标签y
                iter_count += 1
                # pdb.set_trace()
                bs, stock_num = batch_x1.shape[0], batch_x1.shape[1]
                batch_x1 = batch_x1.reshape(-1, batch_x1.shape[-2], batch_x1.shape[-1]).float().to(self.device)
                ...
                _,_, output = self.model(batch_x1, batch_x2)    # (1408,60,10) (1408,1,10)  16x88 60 10
    ...
    def test(self, setting):
        ...
        metrics_builders = [
            metrics_object.MIRRTop1,
            # metrics_object.MIRRTopK,
            metrics_object.RankIC,
            metrics_object.MAE,
            metrics_object.MSE,
            metrics_object.RSEMetric,
            metrics_object.MAPE,
            metrics_object.MSPE,
            metrics_object.Covariance,
            metrics_object.Correlation,
        ]
